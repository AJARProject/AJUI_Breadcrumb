...
Case of 
	...
	: (Form event code:C388=On Clicked:K2:4)
		...
		$nameTemplate:=Form:C1466.bc1.templateName+"_copy"
		
		  //save duplicate
		$templatePath:=getTlocPath (Form:C1466.bc1.templateName+$icon)
		$folder:=Folder:C1567($templatePath;fk platform path:K87:2)
		
		If ($folder.name=Form:C1466.bc1.templateName)
			$folderPref:=Folder:C1567(getTlocPath ($icon);fk platform path:K87:2)
			If ($folder.exists)
				$folder:=$folder.copyTo($folderPref;$nameTemplate)
				$templatePath:=Replace string:C233($templatePath;Form:C1466.bc1.templateName;$nameTemplate)
			Else 
		...
		If (Not:C34($folderNotFound))
			$file:=File:C1566($templatePath+Form:C1466.bc1.templateName+".json";fk platform path:K87:2)
			If ($file.exists)
				If ($file_copy)
				...
				End if 
				Form:C1466.bc1.templateName:=$nameTemplate
				Form:C1466.bc1.Export($nameTemplate;$templatePath)
